...
class AuthPracticePageIntegrationTest @Autowired constructor(
    ...
    fun `Google OAuth 페이지는 공식 버튼과 내부 계정 영수증을 제공한다`() {
        mockMvc.perform(get("/auth-practice/oauth.html"))
            ...
            .andExpect(content().string(containsString("id=\"loginMethodsValue\"")))
            .andExpect(content().string(containsString("id=\"passwordTransfer\"")))
            .andExpect(content().string(containsString("id=\"localPasswordPanel\"")))
            .andExpect(content().string(containsString("id=\"localPasswordForm\"")))
            .andExpect(content().string(containsString("id=\"skipLocalPasswordButton\"")))
            .andExpect(content().string(containsString("Google password")))
            .andExpect(content().string(containsString("GET /auth/me")))
            .andExpect(content().string(containsString("POST /auth/local-password")))
    ...
    fun `SMTP 복구 페이지는 요청과 일회용 token 확정 흐름을 제공한다`() {
        mockMvc.perform(get("/auth-practice/recovery.html"))
            ...
            .andExpect(content().string(containsString("id=\"deliveryChecklist\"")))
            .andExpect(content().string(containsString("Receiver-side evidence")))
            ...
            .andExpect(content().string(containsString("200 RECOVERY_MAIL_SENT")))
            .andExpect(content().string(containsString("422 RECOVERY_MAIL_NOT_SENT")))
            .andExpect(content().string(containsString("429 RECOVERY_MAIL_COOLDOWN")))
            .andExpect(content().string(containsString("424 RECOVERY_MAIL_AUTHENTICATION_FAILED")))
            .andExpect(content().string(containsString("424 RECOVERY_MAIL_DELIVERY_FAILED")))
            .andExpect(content().string(not(containsString("202 Accepted"))))
            .andExpect(content().string(not(containsString("SMTP preflight"))))
            .andExpect(content().string(not(containsString("id=\"authForm\""))))
            .andExpect(content().string(not(containsString("id=\"googleLoginLink\""))))
    }
    ...
    fun `세 실습 페이지의 공통 자산과 전용 script에 인증 경계가 분리되어 있다`() {
        ...
        mockMvc.perform(get("/auth-practice/oauth.js"))
            ...
            .andExpect(content().string(containsString("/auth/local-password")))
            .andExpect(content().string(containsString("data.loginMethods")))
            .andExpect(content().string(containsString("response.status === 204")))
        ...
        mockMvc.perform(get("/auth-practice/recovery.js"))
            ...
            .andExpect(content().string(containsString("response.status === 200")))
            .andExpect(content().string(containsString("RECOVERY_MAIL_SENT")))
            .andExpect(content().string(containsString("RECOVERY_MAIL_NOT_SENT")))
            .andExpect(content().string(containsString("RECOVERY_MAIL_COOLDOWN")))
            .andExpect(content().string(containsString("RECOVERY_MAIL_AUTHENTICATION_FAILED")))
            .andExpect(content().string(containsString("RECOVERY_MAIL_DELIVERY_FAILED")))
            .andExpect(content().string(containsString("deliveryChecklist.hidden = false")))
            .andExpect(content().string(containsString("evidence.setState(\"success\"")))
            .andExpect(content().string(not(containsString("response.status === 202"))))
    }
}
