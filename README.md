...
- Google provider identity를 유지하면서 사용자가 아는 LOCAL 비밀번호를 선택적으로 한 번 등록합니다.
- `/auth/me`의 `loginMethods`로 신규·기존 OAuth 계정의 로그인 수단을 같은 방식으로 확인합니다.
...
│   └── service/Step04LocalPasswordEnrollmentService.kt
├── oauth/
...
│   ├── mail/Step06SmtpRecoveryMailSender.kt
│   └── service/Step05AccountRecoveryService.kt
└── post/
...
- `/auth/me`는 내부 email과 `loginMethods`를 반환합니다. Google 계정은 기본 `GOOGLE`, LOCAL 비밀번호 등록 뒤에는 `GOOGLE + LOCAL`입니다.
- `POST /auth/local-password`는 Bearer JWT의 Principal만 사용하고 요청 body의 email을 받지 않습니다. Google 계정의 최초 등록만 허용하며 성공은 no-store 204, 반복 등록은 409입니다.
- 등록 시 BCrypt password와 `localPasswordEnabled=true`를 저장하되 `authProvider=GOOGLE`과 `providerId`는 유지합니다. provider를 LOCAL로 바꾸면 다음 Google 로그인이 같은 계정을 찾지 못합니다.
- 같은 email의 기존 LOCAL 계정은 이 등록 API로 자동 연결하지 않으며 기존 `link_required` 정책을 유지합니다.
...
- email을 검증·정규화하고 `localPasswordEnabled=true`인 계정에만 메일을 보냅니다. 따라서 LOCAL 가입자와 LOCAL 비밀번호를 추가한 Google 계정이 같은 복구 정책을 사용합니다.
- reset token을 commit한 뒤 실제 `JavaMailSender.send()`를 동기로 호출합니다. 정상 반환하면 no-store 200 `RECOVERY_MAIL_SENT`입니다.
- 계정 없음과 LOCAL 비밀번호 미등록 계정은 같은 no-store 422 `RECOVERY_MAIL_NOT_SENT`, cooldown은 `Retry-After`가 있는 429입니다.
...
- OAuth 컬럼 추가 전에 생성된 계정은 `data.sql`이 `auth_provider = ''`이고 `provider_id`가 없을 때만 시작 시 `LOCAL`로 보정하고 `local_password_enabled=true`로 맞춥니다. 외부 식별자가 있는 계정은 자동 변경하지 않습니다.
...
- 1분 cooldown은 LOCAL 비밀번호를 가진 사용자 단위이며 IP·장치·분산 rate limiter를 대신하지 않습니다.
...
- Gmail 전달성은 받은편지함·스팸함과 원본 보기의 SPF·DKIM·DMARC, From·Return-Path·mailed-by·signed-by를 수동 확인합니다.
- 최초 LOCAL 비밀번호 등록은 이 시퀀스 범위에서 OAuth 직후 메모리 JWT와 1회 상태 전이를 사용합니다. 운영에서는 최근 OAuth 재인증이나 짧은 단일 목적 setup grant를 추가해야 합니다.
...
Google 로그인은 `sub`, 검증된 email 같은 profile을 내부 계정 판단에 사용하고 우리 JWT를 따로 발급합니다. Google 비밀번호는 전달되지 않습니다. 사용자가 자체 로그인도 원하면 OAuth 영수증 안에서 별도의 LOCAL 비밀번호를 선택해 등록합니다. 화면의 Google 표시는 [Google Identity 브랜딩 가이드](https://developers.google.com/identity/branding-guidelines?hl=en)와 Google이 배포한 [공식 G 이미지](https://developers.google.com/static/identity/images/g-logo.png)를 기준으로, 원본 PNG를 색상·비율 변경 없이 로컬 SVG wrapper에 포함합니다(확인일: 2026-07-24).

## Visual Lab

OAuth 계정 판단·선택적 LOCAL 자격 등록과 reset token의 발급·동기 SMTP·수신자 증거·확정 생명주기를 네 가지 조건으로 비교합니다. 애플리케이션과 함께 띄울 때는 다른 port를 사용합니다.
...
- Google identity 보존과 `localPasswordEnabled`를 분리하고, 신규·기존 OAuth 계정이 같은 설정 영수증을 쓰는지 확인합니다.
