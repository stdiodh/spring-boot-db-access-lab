...
1. 같은 email의 로컬 계정과 Google 계정이 만나면 왜 자동 연결하지 않고 명시적 확인을 요구하는가
2. 비밀번호 재설정 메일 요청은 왜 보안 관점에서 조심해야 하는가

## 브랜치 사용 방법

- `main`: 이 레포의 주제, 문서, 브랜치 구조를 안내하는 대표 브랜치
- `05-implementation`: 실습용 starter 브랜치
- `05-answer`: 참고 구현 브랜치

실습은 반드시 `05-implementation`에서 시작합니다.
...
완료 후에는 이 브랜치의 구현과 체크리스트로 비교합니다.
...
실습자는 검증된 외부 로그인 성공 후 사용자 식별과 계정 충돌 처리,
...
- Google 사용자 정보 읽기
- 기존 OAuth 사용자, 계정 연결 필요, 신규 사용자 분기
- OAuth 성공 후 JWT fragment와 redirect 파라미터 정리
