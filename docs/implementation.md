...
| 레거시 계정 보정 | `src/main/resources/data.sql` | 빈 provider·외부 식별자 없음 조건의 LOCAL 보정 |
...
| SMTP 사전검사 | `RecoveryMailReadiness.kt` | 계정 조회 전 연결·인증 확인과 안전한 503 |
...
애플리케이션 시작 시 `data.sql`이 `auth_provider = ''`이고 `provider_id`도 없는 기존 계정만 `LOCAL`로 보정합니다. OAuth 컬럼 추가 전에 생성된 자체 가입 계정을 복구 흐름에 다시 연결하기 위한 idempotent 보정이며, Google 계정과 외부 식별자가 남은 데이터는 변경하지 않습니다.
...
복구 요청은 계정 조회 전에 SMTP 연결·인증 사전검사를 실행합니다. Gmail 인증 값이 없거나 거부되면 `RECOVERY_MAIL_AUTHENTICATION_FAILED`, SMTP 서버에 연결할 수 없으면 `RECOVERY_MAIL_UNAVAILABLE`의 no-store 503을 반환합니다. 이 검사는 모든 유효한 email에 같은 순서로 적용하며 recipient와 reset token을 SMTP에 보내지 않습니다. 사전검사는 실제 발신자 허용, 수신함 도착이나 commit 이후 비동기 전송 성공을 보장하지 않습니다.
...
빈 값, 잘못된 형식, 254자 초과는 400입니다. 유효한 DTO는 계정 조회 전 전역 SMTP 사전검사를 먼저 실행하고, 실패하면 계정 존재와 무관한 같은 503을 반환합니다. 사전검사를 통과한 뒤에는 계정 없음, OAuth 계정, LOCAL 계정과 commit 이후 비동기 발송 실패를 구분하지 않고 `Cache-Control: no-store`와 202를 반환합니다. 202는 mail delivery 성공을 뜻하지 않습니다.
...
- `RecoveryMailReadiness`가 계정 조회 전에 SMTP 연결·인증만 동기로 확인하며, 실패할 때도 계정 service를 호출하지 않습니다.
...
- `@Async`이므로 사전검사 통과 뒤 HTTP 요청은 실제 메시지 전송 완료를 기다리지 않습니다.
- event의 `toString`과 실패 log는 recipient, token, link, SMTP 원인을 노출하지 않습니다.
- commit 이후 SMTP 전송 또는 task 거부가 발생해도 이미 정해진 공개 202 계약을 바꾸지 않습니다.
...
./gradlew test --tests '*SmtpRecoveryMailReadinessTest'
...
자동 테스트는 OAuth 검증·계정 정책·redirect·session 경계, HTML 정적 진입점과 URL 처리 코드 연결, 계정 조회 전 SMTP 503, LOCAL recovery·202·cooldown, token hash·회전·만료·단일 사용, AFTER_COMMIT async dispatch, SMTP 실패·메시지 조립과 최신 04 회귀를 확인합니다. 실제 URL 제거 동작은 브라우저에서도 확인합니다.
...
3. 올바른 SMTP 연결·인증이면 복구 endpoint가 사전검사 뒤 202를 반환하고 메일이 비동기로 도착하는지 확인합니다.
...
6. 앱 비밀번호 누락·오류와 SMTP 연결 실패가 계정 조회 전에 같은 안전한 503으로 표시되는지 확인합니다.
7. 사전검사 통과 뒤 없는 계정과 OAuth 계정도 같은 202인지 확인합니다.
8. credential, 복구 대상 email, reset token, link, SMTP 내부 오류가 로그에 없는지 확인합니다.
...
- 계정 조회 전 SMTP 503과 사전검사 통과 뒤 recovery 202, LOCAL-only, 1분 cooldown, AFTER_COMMIT async mail 경계를 지킵니다.
