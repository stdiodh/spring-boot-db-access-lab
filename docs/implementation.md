...
Recovery request -> LOCAL lock -> hashed token commit -> synchronous SMTP -> 200 / 4xx
...
핵심 production 파일 5개만 `Step` + 2자리 번호 + 기존 PascalCase 파일명으로 표시합니다.
...
Step 4-A와 4-B는 같은 서비스 파일에 있으므로 요청 결과 계약과 confirm transaction을 함께 검증합니다.
...
복구 요청은 LOCAL token을 commit한 뒤 실제 SMTP 호출을 기다립니다. Gmail 인증 값이 없거나 거부되면 no-store 424 `RECOVERY_MAIL_AUTHENTICATION_FAILED`, 그 밖의 전송 실패는 424 `RECOVERY_MAIL_DELIVERY_FAILED`입니다. 정상 200은 SMTP 서버가 요청을 수락한 범위이며 실제 수신함 도착까지 보장하지 않습니다.
...
3. `LOCAL`이 아니어도 같은 422 오류를 사용해 세부 원인은 구분하지 않습니다.
...
Service는 `JavaMailSender`를 직접 알지 않아야 합니다. 200은 `send()`의 정상 반환까지만 증명합니다. 테스트는 sender 또는 `JavaMailSender`를 mock하므로 외부 SMTP에 연결하지 않습니다.
...
자동 테스트는 OAuth 검증·계정 정책·redirect·session 경계, HTML 정적 진입점과 URL 처리 코드 연결, LOCAL recovery의 200/422/429/424, token hash·회전·만료·단일 사용, commit 이후 동기 SMTP, 실패 token 정리와 최신 04 회귀를 확인합니다. 실제 URL 제거 동작은 브라우저에서도 확인합니다.
...
7. 없는 계정과 OAuth 계정은 같은 422, cooldown은 `Retry-After`가 있는 429인지 확인합니다.
8. 424 뒤 이번 요청의 token이 정리되어 즉시 재시도할 수 있는지 확인합니다.
9. credential, 복구 대상 email, reset token, link, SMTP 내부 오류가 로그에 없는지 확인합니다.
...
- recovery 200/422/429/424, LOCAL-only, 1분 cooldown, commit 이후 동기 SMTP와 실패 token 정리 경계를 지킵니다.
