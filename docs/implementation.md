...
5. `security/JwtAuthentication.kt`에서 JWT 발급·검증과 인증 필터를 연결합니다.
6. `security/SecurityConfig.kt`에서 공개 API와 보호 API를 구분합니다.
7. `service/PostService.kt`에서 게시글 소유권 검사를 완성합니다.
...
`User`가 회원가입 정보를 안전하게 저장할 수 있는지 확인합니다. 저장과 조회에 필요한 `UserRepository`는 이미 제공되므로 수정하지 않습니다.
...
`JwtAuthentication.kt`에 함께 있는 `JwtTokenProvider`와 `JwtAuthenticationFilter`에서 토큰 검증 흐름을 확인합니다.
