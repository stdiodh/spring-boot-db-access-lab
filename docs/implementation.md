...
이번 answer는 기존 CRUD 요청이 안전하게 들어오고, 실패했을 때 일정한 응답으로 나가도록 만든 비교 기준입니다.
...
1. `PostCreateRequest.kt`, `PostUpdateRequest.kt`의 기본 검증을 확인합니다.
2. `PostResponse.kt`에서 응답 DTO 변환을 확인합니다.
3. `PostService.kt`에서 없는 게시글을 비즈니스 예외로 분리하는지 확인합니다.
4. `ErrorResponse.kt`에서 실패 응답 구조를 확인합니다.
5. `GlobalExceptionHandler.kt`에서 검증 실패와 게시글 없음 응답을 확인합니다.
6. Swagger에서 정상 요청과 실패 요청을 비교합니다.

## 3. Step 1. 요청 DTO 검증 확인

### 해야 할 일

생성/수정 요청 DTO의 필드에 기본 검증이 연결되어 있는지 확인합니다.
...
없는 게시글 조회/수정/삭제 상황이 `PostNotFoundException`으로 구분되는지 확인합니다.
...
## 6. Step 4. 실패 응답 통일 확인

### 해야 할 일

`GlobalExceptionHandler`에서 검증 실패와 게시글 없음 실패를 `ErrorResponse`로 변환하는지 확인합니다.
...
- starter와 비교할 때 DTO annotation, Controller의 `@Valid`, Service 예외, ExceptionHandler 순서로 확인합니다.
- 힌트가 필요하면 "어디서 막는가"와 "어떤 응답으로 바꾸는가"를 묻게 합니다.
- 다음 JWT 시퀀스로 넘어가기 전 입력 안전성 기준을 설명하게 합니다.
