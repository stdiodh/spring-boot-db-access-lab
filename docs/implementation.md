...
- "fixture 는 테스트 본문을 짧게 해서 검증 흐름을 더 잘 보이게 만든다."
- "fixture 는 로직이 아니라 준비 코드다."
...
이 단계는 "실패도 테스트 대상"이라는 감각을 잡는 핵심 구간입니다.
...
여기서는 토큰 문자열 그 자체보다
"토큰 안에 기대한 사용자 식별값이 들어 있는가"를 확인한다는 점이 중요합니다.
...
이 단계는 "사용자를 찾았더라도 인증은 실패할 수 있다"는 구조를 분리해서 보여줍니다.

## 왜 이번에는 service test 만 하는가

이번 시퀀스는 테스트 입문 단계이므로,
모든 계층을 한 번에 검증하기보다 service 범위만 먼저 붙잡습니다.
...
controller, repository, integration 테스트는 중요하지만,
이번 단계에서는 범위를 좁혀 service 테스트 감각을 먼저 만드는 것이 목표입니다.
...
4. 실패하면 given, mock 설정, 기대값을 다시 확인합니다.
5. 수정 후 다시 `./gradlew test`를 실행합니다.
...
- 왜 지금은 service test 에 집중하는지 설명할 수 있나요?
...
- 왜 지금은 integration 이 아니라 service test 인지 설명할 준비가 되었는가
