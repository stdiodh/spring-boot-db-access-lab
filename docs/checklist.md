...
- [ ] SMTP 호출은 LOCAL token transaction이 commit된 뒤 request thread에서 동기로 실행합니다.
- [ ] SMTP 정상 반환은 no-store 200 `RECOVERY_MAIL_SENT`입니다.
- [ ] 계정 없음과 OAuth 계정은 같은 no-store 422 `RECOVERY_MAIL_NOT_SENT`입니다.
...
- [ ] LOCAL 사용자별 1분 cooldown 안에는 token과 mail command를 재발급하지 않습니다.
...
- [ ] LOCAL-only recovery와 cooldown 경계 테스트가 있습니다.
...
- [ ] 없는/OAuth 계정은 같은 422, cooldown은 429와 `Retry-After`인지 확인했습니다.
- [ ] LOCAL 성공은 SMTP 호출이 반환된 뒤에만 200인지 확인했습니다.
...
- 실제 메일보다 200의 증거 범위, LOCAL-only, token commit과 동기 SMTP 경계를 먼저 설명하게 합니다.
