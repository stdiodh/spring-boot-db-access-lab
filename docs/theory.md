...
- 이 코드는 Google 응답을 우리 코드에서 쓰기 쉬운 값으로 바꾸는 출발점입니다.
- 학생은 `email`과 `sub`가 왜 중요한지 먼저 보면 좋습니다.
...
- 이 코드는 "외부 로그인도 결국 우리 사용자로 정리해야 한다"는 핵심 장면입니다.
- 여기서는 provider + providerId 기준과 email 기준 두 갈래를 먼저 보세요.
...
- 이 코드는 OAuth 로그인 성공이 브라우저 화면까지 어떻게 이어지는지 보여줍니다.
- 학생은 "Google 로그인 성공"에서 끝나는 것이 아니라, 우리 프론트로 결과를 정리해 넘긴다는 점을 잡으면 됩니다.
...
- **짧은 상황 예시**  
  사용자가 Google 버튼을 누르면 Google이 로그인 과정을 처리하고, 성공 후 우리 서버로 돌아옵니다.
...
- **뜻**  
  외부 인증을 제공하는 서비스입니다.
- **왜 중요한가**  
  어떤 제공자를 통해 로그인했는지 알아야 사용자 연결 기준이 선명해집니다.
- **이번 코드에서는 어디에 보이는가**  
  `GOOGLE`, `registrationId`, `profile.provider`
- **짧은 상황 예시**  
  지금은 Google 하나만 쓰지만, 나중에는 provider 값으로 구분할 수 있습니다.
...
- **짧은 상황 예시**  
  email이 바뀌더라도 provider id 기준으로 같은 사용자를 찾을 수 있습니다.
...
- **짧은 상황 예시**  
  이미 있는 로컬 사용자를 Google 계정과 연결하거나, 없으면 새 사용자를 만듭니다.
