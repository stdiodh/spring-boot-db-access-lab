...
OAuth 응답의 email은 `email_verified`가 확인된 경우만 사용합니다. 같은 email의 로컬 계정은 자동 연결하지 않고 명시적인 연결 확인 절차가 필요합니다. 데모 JWT는 서버 로그와 referrer에 남는 query 대신 URL fragment로 전달하지만, 운영 환경에서는 일회용 code 교환이나 HttpOnly cookie 같은 더 안전한 전달 방식을 사용해야 합니다.
