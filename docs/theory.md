...
- 실습자는 `@PostMapping("/signup")`, `@PostMapping("/login")`, `@GetMapping("/me")`를 먼저 보면 좋습니다.
...
- 실습자가 기억해야 할 핵심은 "로그인은 사용자 확인이 끝난 뒤 토큰을 발급하는 흐름"이라는 점입니다.
...
- 실습자는 "토큰은 로그인 응답에서 끝나는 값이 아니라 다음 요청에서 다시 읽히는 값"이라는 감각을 잡으면 됩니다.
