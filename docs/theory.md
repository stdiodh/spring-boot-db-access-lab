...
- 여기서는 `email`, `providerId`가 왜 둘 다 필요한지 먼저 봅니다.
...
- 이 코드는 단순 저장이 아니라 계정 연결 정책의 시작점입니다.
...
- Google이 인증을 대신 해줘도, 다음 요청을 구분하는 토큰은 우리 서비스가 다시 발급합니다.
...
- SMTP 파트의 핵심은 "메일을 보낸다"보다 "어떤 링크를 어떤 값으로 만들까"에 가깝습니다.
...
- OAuth2, providerId, SMTP, password reset link는 각각 무엇인가요?
...
- Google 로그인 이후에도 왜 우리 서비스 JWT가 다시 필요한가요?
