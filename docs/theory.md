...
이번 시퀀스를 마치면 다음을 확인합니다. 제공된 연결 코드와 58개 테스트는 Step01부터 Step08까지의 TODO와 같은 계약을 사용합니다. 시작 상태의 테스트 실패를 테스트 수정으로 숨기지 않고, TODO를 모두 구현한 뒤 전체 테스트가 통과해야 합니다.
...
- 58개 테스트는 TODO와 같은 계약을 검증하며, 테스트를 완화하는 방식으로 해결하지 않게 안내합니다.
