...
> 이번 시퀀스는 03의 Validation을 인증 입력에 다시 적용하고, 회원가입, 로그인, JWT 발급, JWT 검증, 공개 API와 보호 API 구분을 다룹니다. 목표는 토큰 문자열 자체를 외우는 것이 아니라 "어떤 입력을 받을 수 있는가"와 "누가 요청했는가"를 서버가 어떻게 판단하는지 설명하는 것입니다.
