...
이번 answer는 요청 DTO의 기본 검증, 비즈니스 예외 분리, 공통 실패 응답 변환이 연결된 상태를 보여줍니다.

## 2. 기존 방식의 한계

검증을 Service 안쪽 조건문으로만 처리하면 요청 검증과 비즈니스 처리가 섞입니다. 예외가 파일마다 다른 모양으로 응답되면 클라이언트도 실패를 안정적으로 처리하기 어렵습니다.
...
검증 실패와 게시글 없음 실패는 원인은 다르지만 `ErrorResponse`라는 같은 틀로 응답됩니다.
...
- starter와 비교할 때 DTO annotation, `@Valid`, ExceptionHandler 응답 변환을 순서대로 확인하게 합니다.
- 멘티가 Validation을 Service 내부 조건문으로만 이해하면 요청 초입에서 막는다는 관점으로 되돌립니다.
- 커스텀 Validation은 이번 필수 구현 범위 밖임을 설명하고, 기본 검증으로 막기 어려운 규칙이 생길 때 확장한다고 연결합니다.
