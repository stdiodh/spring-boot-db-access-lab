...
- Google provider identity / 선택적 LOCAL 비밀번호 자격
...
| LOCAL 자격 등록 | Bearer JWT, 새 password | Google 계정·미등록 상태 잠금 확인 | BCrypt password, `localPasswordEnabled=true`, `204` |
| 복구 요청 | LOCAL 비밀번호 사용 가능 email | cooldown 확인, raw/hash 분리, token commit | 동기 발송용 mail command |
| SMTP | commit된 mail command | 실제 `JavaMailSender.send()` 호출 | `200` 또는 원인을 구분한 `424` |
| 수신자 확인 | 받은 메일과 원본 헤더 | 받은편지함·스팸, SPF·DKIM·DMARC 정렬 확인 | 수동 전달성 증거 |
...
Google 비밀번호는 OAuth profile에 포함되지 않으며 우리 서버로 전달되지 않습니다. 신규 OAuth 사용자를 저장할 때 DB의 NOT NULL 계약을 만족하려고 서버가 만든 무작위 값의 BCrypt hash를 둘 수는 있지만, 그 값은 사용자가 아는 LOCAL 비밀번호가 아니며 자체 로그인에 사용할 수 없습니다.

이번 시퀀스는 `localPasswordEnabled`를 provider identity와 분리합니다. LOCAL 가입은 `true`, 신규 Google 계정은 `false`이며, 인증된 Google 사용자가 `POST /auth/local-password`로 직접 고른 비밀번호를 최초 한 번 등록하면 BCrypt hash와 함께 `true`가 됩니다. 이때 `authProvider=GOOGLE`과 `providerId`는 바꾸지 않습니다. 이를 LOCAL로 덮어쓰면 다음 Google callback의 `(GOOGLE, providerId)` 조회가 실패해 같은 email 충돌로 되돌아갑니다.

등록 요청은 email을 받지 않고 Bearer JWT의 Principal만 사용합니다. 사용자 행을 잠근 뒤 Google 계정이면서 아직 LOCAL 자격이 없는 경우에만 8~64자 비밀번호를 저장합니다. 성공은 no-store 204, 반복 등록이나 부적절한 상태는 409입니다. 같은 email의 기존 LOCAL 계정과 외부 identity 연결은 이 API 범위가 아니며 계속 `link_required`로 끝납니다.

OAuth 컬럼을 추가하기 전에 만들어진 자체 가입 계정은 MySQL schema update 과정에서 `auth_provider`가 빈 문자열로 남을 수 있습니다. 시작 시 실행되는 `data.sql`은 `auth_provider = ''`이고 `provider_id`도 없는 행만 `LOCAL`로 바꾸고 기존 LOCAL 행의 `local_password_enabled`를 `true`로 맞춥니다. 외부 식별자가 있는 행까지 추측해 바꾸면 OAuth 계정을 LOCAL로 오인할 수 있으므로 자동 보정 범위를 넓히지 않습니다.
...
sequenceDiagram
    ...
    participant Local as LocalPasswordEnrollmentService
    ...
    Browser->>Browser: GET /auth/me로 loginMethods 확인
    opt 사용자가 LOCAL 로그인을 추가
        Browser->>Local: POST /auth/local-password + Bearer JWT
        Local-->>Browser: 204 · GOOGLE + LOCAL
    end
```
...
형식이 유효한 `POST /account-recovery/password-reset`은 계정, `localPasswordEnabled`와 cooldown을 먼저 판단합니다. LOCAL 비밀번호를 사용할 수 있는 계정이면 provider 종류와 관계없이 token transaction을 commit한 뒤 같은 HTTP 요청에서 실제 SMTP 호출이 끝날 때까지 기다립니다.

- SMTP 호출 정상 반환: no-store 200 `RECOVERY_MAIL_SENT`
- 계정 없음 또는 LOCAL 비밀번호 미등록: no-store 422 `RECOVERY_MAIL_NOT_SENT`
...
이 계약은 실습에서 실패 지점을 관찰하기 위해 200/422/429/424를 구분합니다. 그 결과 reset 가능한 LOCAL 자격과 cooldown 상태를 추측할 수 있으므로 계정 열거 방어가 필요한 공개 운영 API에 그대로 적용하지 않습니다.
...
4. LOCAL 비밀번호를 가진 사용자당 token 행은 하나이며 새 발급 시 같은 행을 회전합니다. 그러면 이전 raw token은 즉시 매칭되지 않습니다.
...
LOCAL 비밀번호를 가진 사용자별 기본 재요청 cooldown은 1분입니다. 활성 token을 발급한 뒤 1분 전까지는 새 token과 메일을 만들지 않으며 정확히 1분 경계에서는 다시 허용합니다.
...
- 최초 LOCAL 비밀번호 등록은 OAuth 직후 메모리 JWT와 1회 상태 전이에 의존합니다. 운영에서는 최근 OAuth 재인증 또는 짧은 단일 목적 setup grant가 필요합니다.
...
- LOCAL 자격 최초 등록·반복 409·provider identity 보존과 `loginMethods`
...
- 실제 SMTP 인증, TLS, 발신자 정책, 원본 헤더와 수신함 도착
...
- GOOGLE provider identity와 LOCAL 비밀번호 자격을 분리하는 이유
- `/auth/me`의 `loginMethods`가 화면 판단의 근거인 이유
...
- LOCAL 비밀번호 사용 가능 여부와 `RecoveryMailSender` 분리
- SMTP 200과 Gmail 원본 헤더·받은편지함 배치가 다른 증거인 이유
