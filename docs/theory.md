...
이 과정에서 실습자는 "왜 Repository가 필요한지", "왜 Entity가 따로 있는지", "왜 계층을 나누는지"를 코드와 실행 결과로 같이 확인하게 됩니다.
...
- 여기서는 "아, 이제 메모리 리스트가 아니라 테이블로 가는구나"라는 감각을 잡으면 됩니다.
...
도메인 관계를 객체와 쿼리 양쪽에 동시에 영향을 주는 설정이라는 점입니다.

### 2. N+1 문제

JPA를 처음 쓸 때 가장 자주 듣는 성능 문제 중 하나가 `N+1`입니다.
이 개념은 이름만 외우면 잘 안 남고, 문제 코드가 왜 자연스러워 보이는지를 같이 봐야 이해가 됩니다.
...
이 문제를 줄이는 대표 예시 중 하나가 `fetch join`입니다.
핵심은 “게시글만 먼저 읽고, 댓글을 나중에 하나씩 다시 읽는 방식”을
“처음 조회할 때 필요한 연관 데이터도 함께 읽는 방식”으로 바꾸는 것입니다.

예를 들면 Repository 코드를 아래처럼 가져갈 수 있습니다.
...
- `Post`를 조회할 때
- `comments`도 같이 가져오고
- 게시글마다 댓글을 다시 읽는 추가 조회를 줄여보자는 것

Service에서는 이런 식으로 조회 메서드를 바꿔 연결할 수 있습니다.
...
- 이전: `findAll()` 후에 댓글을 각 게시글마다 다시 읽을 수 있음
- 이후: `findAllWithComments()`로 필요한 연관 데이터를 한 번에 가져오려 함

즉 해결 방향은 “연관 데이터를 접근하는 코드가 문제”가 아니라,
연관 데이터를 어떤 조회 전략으로 읽어오느냐가 성능을 크게 바꾼다는 점입니다.

실무에서는 상황에 따라 아래 같은 방법도 함께 봅니다.

- `fetch join`
- `EntityGraph`
- DTO 조회 최적화

지금 단계에서는 “문제를 만들 수 있는 코드”와
“그 문제를 줄이기 위한 조회 코드 모양”까지 연결해서 보는 것이 중요합니다.

### 4. 이번 시퀀스에서는 어디까지 다루는가
...
- 그것이 N+1의 시작이라는 점
- `fetch join` 같은 해결 코드가 어떤 방향인지 이해하는 것

다만 이번 메인 구현은 여전히 단일 테이블 CRUD입니다.
즉 지금 단계에서는 `Post-Comment` 연관관계를 실제 코드에 모두 붙여 구현하기보다,
문제 코드 -> 느려지는 이유 -> 해결 코드 예시까지 설명할 수 있는 상태를 목표로 잡습니다.
